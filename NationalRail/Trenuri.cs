...
{
    public partial class Trenuri : Form
    {
        string tableName = "TRENURI"; //AMOGUS
        string identifier = "id_tren"; //AMOGUS
        ...
        {
            string[] expectedHeaders = { "id_tren", "vagoane", "masa", "v_max" }; //AMOGUS
        ...
        private void add_btn_Click(object sender, EventArgs e)
        {
            ...
            string vagoane = wagon_field.Text;
            string mass = mass_field.Text;
            string v_max = speed_field.Text;

            // Validate the input fields
            if (string.IsNullOrEmpty(vagoane) || string.IsNullOrEmpty(mass) || string.IsNullOrEmpty(v_max))
            {
                MessageBox.Show("Completați toate câmpurile.", "Atenție", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (!double.TryParse(mass, out _) || !double.TryParse(v_max, out _))
            {
            ...
            jTable1.Rows.Add(randomID, vagoane, mass, v_max);
        ...
        private void modify_btn_Click(object sender, EventArgs e)
        {
            if (jTable1.SelectedRows.Count == 1)
            {
                ...
                string mass = mass_field.Text;
                string speed = speed_field.Text;

                // Validate the input fields
                if (string.IsNullOrEmpty(wagon) || string.IsNullOrEmpty(mass) || string.IsNullOrEmpty(speed))
                {
                    MessageBox.Show("Completați toate câmpurile.", "Atenție", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                selectedRow.Cells["vagoane"].Value = wagon;
                selectedRow.Cells["masa"].Value = mass;
                selectedRow.Cells["v_max"].Value = speed;
        ...
        private void ApplyFilter()
        {
            ...
            if (double.TryParse(low_field.Text, out double lowSalary) && double.TryParse(high_field.Text, out double highSalary))
            {
                ...
                foreach (DataGridViewRow row in jTable1.Rows)
                {
                    if (row.Cells["masa"].Value != null && double.TryParse(row.Cells["masa"].Value.ToString(), out double salary))
                    {
        ...
        private void jTable1_CellClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                speed_field.Text = selectedRow.Cells["v_max"].Value.ToString();
                wagon_field.Text = selectedRow.Cells["vagoane"].Value.ToString();
                mass_field.Text = selectedRow.Cells["masa"].Value.ToString();
            }
        }
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    foreach (DataGridViewRow row in jTable1.Rows)
                    {
                        ...
                        if (!row.IsNewRow)
                        {
                            ...
                            string wagon = row.Cells["vagoane"].Value.ToString();
                            string mass = row.Cells["masa"].Value.ToString();
                            string speed = row.Cells["v_max"].Value.ToString();

                            // Insert the row into the database table
                            string insertQuery = $"INSERT INTO {tableName} ({identifier}, vagoane, masa, v_max) " +
                                $"VALUES ('{id}', '{wagon}', '{mass}', '{speed}')";
